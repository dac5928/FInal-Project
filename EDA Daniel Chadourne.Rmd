...
My second data set will be the World Cities data-set from the DataComputing package. As my research question also relates to population, understanding which countries have the most populous cities.I found this data-set through our work with it during the DC exercises assignments. Although we never directly utilized it, it is similar in structure to that of the ZipGeography data-set with the exception that it focuses on cities in countries other than the United States. As my research project has an international focus, it was important to utilize a data-set that reflected that focus.This data-set was collected over the course of several years during the first two decades of the 21st century. The data was also collected by researchers who used census data, and other population studies to create this data-set. 
...
In the data-set, each case represents on city in a country for a certain date. Some of the variables I intend to use are the country abbreviation, the population, and "countryRegion" in order to create a chloropleth map of the data-set. I plan to use this resulting map to better understand the number of major trade locations in each country, and to determine if the number of cities correlates to the economic power of country. 

```{r}
WorldCities %>% summarise(n())
```
In the data-set, there are 23,018 cases, so there is plenty of data for my project and it also includes data for several of the countries I will be looking at intensely; China, the US, and Russia.

### Plots 


```{r}
WorldCities %>% filter(country == "US") %>% ggplot(aes(y = population, x = lubridate::mdy(date))) + geom_point() + aes(color = region)
```
As you can see in the above plot, there is a higher distribution of the US population on the coasts of the country. While this is to be expected, this also indicates that the areas where the most economic activity is occurring is within these regions of the country. 

```{r}
WorldCities %>% filter(country == "CN") %>% ggplot(aes(y = population, x = lubridate::mdy(date))) + geom_point() + aes(color = region)
```
In comparison to the US, a large number of cities in China are more heavily concentrated near the Pacific Ocean. This would suggest that while China has several major trade hubs, the distribution of economic success in the country is not as even. However, there appears to be more of these trade-hubs in china than in the US. Details like this will also allow me to better understand where the economic power of the US' "challengers" comes from, adn will provide more insight into the accuracy of the CINC scores in the NMC v5.0 dataset.

```{r}
pair <- c("CN", "US")
WorldCities %>% filter(country %in% pair) %>% ggplot(aes(y = population, x = lubridate::mdy(date))) + geom_point() + aes(color = country)
```
This final above plot shows that both the US and China have both seen large increased in population in their major cities, however, it also shows that US cities have lower populations than those of China, and that in recent years more Chinese cities have significantly increased in population than those of the US. This information and similar commands will help me identify other "challengers" by population density. Similarly, I will also be able to better understand the distribution of urban populations that is listed in the NMC v5.0 data-set. 
