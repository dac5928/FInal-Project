...
```{r}
WorldCities %>% filter(country == "US") %>% ggplot(aes(x = population, y = lubridate::mdy(date))) + geom_point() + aes(color = region)
```
...
```{r}
WorldCities %>% filter(country == "CN") %>% ggplot(aes(x = population, y = lubridate::mdy(date))) + geom_point() + aes(color = region)
```
