...
```{r}
NMC_5.0 %>% mutate(x = cinc) %>% mutate(mean = Mean_CINC(x))
```
...
```{r}
Countries_2011 <- NMC_5.0 %>% filter(year == '2012') %>% rename(Military_Personel = milper) %>% rename(Military_Expenditure = milex)
```
...
```{r, warning = FALSE, message = FALSE}
Us_and_China <- US %>% full_join(China)
```

```{r}
Us_and_China %>% ggplot(aes(y = population, x = lubridate::mdy(date))) + geom_point() + aes(color = country) + xlab("Year")
```
